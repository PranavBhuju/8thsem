...
exports.showJobs = async (req, res, next) => {
    ...
    try {
        const jobs = await Job.find({ ...keyword, jobType: categ }) .sort({ createdAt: -1 })  .populate("jobType", "jobTypeName")
        .populate("user", "firstName lastName").skip(pageSize * (page - 1)).limit(pageSize)
        res.status(200).json({
            success: true,
            jobs,
            page,
